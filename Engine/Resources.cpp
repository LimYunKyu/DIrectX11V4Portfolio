	...
	D3D11_BIND_FLAG flags, D3D11_USAGE usage, Vec4 clearColor )
{
	...
	texture->Create(format, width, height, flags, usage, clearColor);
	Add(name, texture);

	return texture;
}

shared_ptr<Texture> Resources::CreateTextureFromResource(const wstring& name, ID3D11Texture2D* tex2D)
{
	shared_ptr<Texture> texture = make_shared<Texture>();
...
void Resources::CreateDefaultMaterial()
{
	...
	// PointLight
	{
		const WindowInfo& window = GEngine->GetWinfo();
